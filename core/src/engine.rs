...
pub enum EventType {
    Arrival { path: Vec<NodeId>, start_time: u64, timeout: u64 },
    ProcessComplete { success: bool, start_time: u64, path: Vec<NodeId>, timeout: u64 },
    Response { path: Vec<NodeId>, start_time: u64, success: bool, timeout: u64 },
    GenerateNext { generation_id: u64 },
}
...
impl Eq for Event {}
impl PartialOrd for Event { fn partial_cmp(&self, other: &Self) -> Option<std::cmp::Ordering> { self.time.partial_cmp(&other.time) } }
impl Ord for Event { fn cmp(&self, other: &Self) -> std::cmp::Ordering { self.time.cmp(&other.time) } }
...
impl Simulation {
    pub fn new() -> Self {
        Self { time: 0, components: HashMap::new(), events: BinaryHeap::new(), success_count: 0, failure_count: 0, latencies: Vec::new() }
    }
