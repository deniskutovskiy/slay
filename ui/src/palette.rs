...
pub fn render_palette(ui: &mut egui::Ui, app: &mut SlayApp) {
    ...
    ui.with_layout(egui::Layout::bottom_up(egui::Align::Center), |ui| {
        ui.add_space(15.0);
        if ui.button(egui::RichText::new("🗑 Clear Canvas").color(COLOR_CRITICAL)).clicked() { app.reset(); }

        ui.add_space(10.0);
        if ui.add_sized([ui.available_width(), 35.0], egui::Button::new("💾 Save Design").fill(COLOR_ACCENT.gamma_multiply(0.2))).clicked() {
            app.save_state();
        }

        ui.add_space(10.0);
        let btn = if app.is_running { "⏸ Stop" } else { "▶ Start" };
