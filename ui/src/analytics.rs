...
impl egui::Widget for SparklineWidget<'_> {
    fn ui(self, ui: &mut egui::Ui) -> egui::Response {
        ...
        if ui.is_rect_visible(rect) {
            ...
            if self.data.len() >= 2 {
                let values: Vec<f32> = self.data.iter().map(|p| (self.field_extractor)(p)).collect();
                let max_val = values.iter().copied().fold(0.0, f32::max).max(0.001);

                let points: Vec<egui::Pos2> = values.iter().enumerate().map(|(i, &v)| {
                    let x = graph_rect.left() + (i as f32 / (values.len() - 1) as f32) * graph_rect.width();
                    let y = graph_rect.bottom() - (v / max_val) * (graph_rect.height() * 0.8) - 2.0;
                    egui::pos2(x, y)
                }).collect();
                ...
                painter.add(egui::Shape::convex_polygon(shape_points, self.color.gamma_multiply(0.15), egui::Stroke::NONE));

                // Line
                painter.add(egui::Shape::line(points, egui::Stroke::new(1.5, self.color)));
            } else {
                painter.text(graph_rect.center(), egui::Align2::CENTER_CENTER, "ANALYZING...", egui::FontId::proportional(10.0), COLOR_TEXT_DIM);
            }

            // Labels - Placed in the reserved top area
            painter.text(rect.left_top() + egui::vec2(8.0, 4.0), egui::Align2::LEFT_TOP, &self.label, egui::FontId::proportional(10.0), COLOR_TEXT_DIM);
            painter.text(rect.right_top() + egui::vec2(-8.0, 4.0), egui::Align2::RIGHT_TOP, &self.current_value_text, egui::FontId::proportional(13.0), self.color);
        }
